...
class RollingAnalyzer:
    ...
    def analyze(self) -> Analysis:
        ...
        return Analysis(
            ...
            next_round_estimate=estimate,
            estimate_range=range_,
            status="ok",
        )
    ...
    def _next_round_estimate(values, observed_target_percentage):
        ...
        spread = 2.0
        low = max(75.0, round(confidence - spread, 2))
        high = min(93.0, round(confidence + spread, 2))
        return round(float(point), 4), (low, high)

def analysis_dict(analysis):
    result = asdict(analysis)
    result["estimate_label"] = (
        "Model-generated probability/confidence estimate based on historical statistics"
    )
